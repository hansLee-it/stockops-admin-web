...
const ALERT_LEVEL_CONFIG: Record<AlertLevel, { label: string; color: string; bgColor: string; icon: React.ComponentType<{ className?: string }> }> = {
  CRITICAL: { label: 'Critical', color: 'text-white', bgColor: 'bg-error', icon: AlertTriangle },
  WARNING: { label: 'Warning', color: 'text-white', bgColor: 'bg-warning', icon: AlertCircle },
  NOTICE: { label: 'Notice', color: 'text-neutral-900', bgColor: 'bg-yellow-400', icon: Clock },
  INFO: { label: 'Info', color: 'text-white', bgColor: 'bg-info', icon: Info },
}
...
export function ExpiryPage() {
  ...
  const summaryCards = [
    { key: 'critical' as const, label: 'Critical', count: summary?.critical ?? 0, color: 'bg-error', icon: AlertTriangle },
    { key: 'warning' as const, label: 'Warning', count: summary?.warning ?? 0, color: 'bg-warning', icon: AlertCircle },
    { key: 'notice' as const, label: 'Notice', count: summary?.notice ?? 0, color: 'bg-yellow-400', icon: Clock },
    { key: 'info' as const, label: 'Info', count: summary?.info ?? 0, color: 'bg-info', icon: Info },
  ]
      ...
      <div className="bg-white p-4 rounded-lg shadow mb-6">
        <div className="flex items-center gap-4">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm text-neutral-600">Show acknowledged</span>
          </label>
          {selectedLevel !== 'ALL' && (
            <button
              type="button"
            ...
            <table className="w-full">
              <thead className="bg-neutral-50 border-b border-neutral-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-sm font-medium text-neutral-700">Days Left</th>
